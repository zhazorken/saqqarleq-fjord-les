...
def plot_face(prefix, d, out):
    ...
    xdim, ydim = dim_like(v, "x"), dim_like(v, "y")
    x, y = ds[xdim].values, ds[ydim].values
    speed = np.hypot(u.transpose(xdim, ydim).values, v.transpose(xdim, ydim).values) if u is not None \
            else np.abs(v.transpose(xdim, ydim).values)
