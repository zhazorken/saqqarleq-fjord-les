...
# depth (~ -15 m), balanced by an outflow sponge at the fjord mouth. The near-terminus rising plume
...
function parse_cli()
    cli = Dict{String,Any}(
        ...
        "y_src" => 150.0, "sig_src" => 60.0, "z_src_bot" => -17.0, "src_gain" => 1.0, "src_top" => 0.0, "z_nb" => 0.0,   # src_top=band top; z_nb<0 => inject neutral-density water that settles to that depth
...
const SRC_TOP   = cli["src_top"]   # top of the source band [m] (0 = surface)
const Z_NB      = cli["z_nb"]      # if <0: inject T/S at the ambient density of this depth (negatively buoyant → settles here, radiating waves)
#---
...
# Negatively-buoyant source: inject T/S at the ambient DENSITY of Z_NB, so a shallow source is
# denser than the fresh surface skin and sinks to Z_NB; the sinking adjustment radiates the internal tide.
const T_NB = Z_NB < 0 ? T∞(Z_NB) : 0.0
const S_NB = Z_NB < 0 ? S∞(Z_NB) : 0.0
#---
...
@inline function v_tgt(z)
    if z < -140 || z > 0
        return 0.0
    elseif z <= -120
        return (-0.03) * (z + 140) / 20                 # deep taper to a weak return flow
    elseif z <= -30
        return -0.03
    elseif z <= -15
        return -0.03 + (0.34 - (-0.03)) * (z + 30) / 15  # (-30,-0.03)→(-15,0.34): peak at -15
    else
        return 0.34 + (0.0 - 0.34) * (z + 15) / 15       # (-15,0.34)→(0,0)
    end
end
@inline function T_tgt(z)
    if z < -30
        return T∞(z)                                     # ambient in the deep
    elseif z <= -17
        return T∞(z) + (T∞(-15.0) - T∞(z)) * (z + 30) / 13   # blend ambient → the 15 m value by -17
    else
        return T∞(-15.0)                                  # injection band held at the 15 m ambient T
    end
end
@inline function S_tgt(z)
    if z < -30
        return S∞(z)                                     # ambient in the deep
    elseif z <= -17
        return S∞(z) + (S∞(-15.0) - S∞(z)) * (z + 30) / 13   # blend ambient → the 15 m value by -17
    else
        return S∞(-15.0)                                  # injection band at the 15 m ambient S (sets density)
    end
end
#---

#+++ Interior source region + mouth sponges (CLOSED domain — no open boundary)
# The fjord model receives the plume as a lateral OUTFLOW at the neutral-buoyancy depth (~ -15 m,
...
@inline in_src(x, y, z) = (9800 <= x <= 9900) && (y <= Y_SRC) && (z >= Z_SRC_BOT) && (z <= SRC_TOP)
